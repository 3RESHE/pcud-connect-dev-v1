...
<div class="bg-gray-50 min-h-screen">
    ...
    <div class="max-w-7xl mx-auto py-6 px-4 sm:px-6 lg:px-8">
        ...
        <div class="mb-8 flex flex-col sm:flex-row sm:justify-between sm:items-start gap-4">
            ...
            <a href="{{ route('alumni.applications.index') }}" class="px-6 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700 font-medium transition-colors whitespace-nowrap text-center">
                View Applications
            </a>
        </div>
        ...
        <div class="bg-white rounded-lg shadow-sm p-4 sm:p-6 mb-8">
            <form action="{{ route('alumni.jobs.index') }}" method="GET">
                <!-- Search Bar Row -->
                <div class="mb-4">
                    <div class="relative">
        ...
        <div class="mb-8 overflow-x-auto">
            <div class="flex gap-2 pb-2">
                <a href="{{ route('alumni.jobs.index') }}" class="@if(!request()->filled('job_type')) bg-primary text-white @else bg-white border border-gray-300 text-gray-700 @endif px-4 py-2 rounded-full text-sm hover:bg-gray-50 transition-colors whitespace-nowrap">
                    All Jobs
                </a>
                <a href="{{ route('alumni.jobs.index', ['job_type' => 'fulltime']) }}" class="@if(request('job_type') === 'fulltime') bg-primary text-white @else bg-white border border-gray-300 text-gray-700 @endif px-4 py-2 rounded-full text-sm hover:bg-gray-50 transition-colors whitespace-nowrap">
                    Full-time
                </a>
                <a href="{{ route('alumni.jobs.index', ['job_type' => 'parttime']) }}" class="@if(request('job_type') === 'parttime') bg-primary text-white @else bg-white border border-gray-300 text-gray-700 @endif px-4 py-2 rounded-full text-sm hover:bg-gray-50 transition-colors whitespace-nowrap">
                    Part-time
                </a>
                <a href="{{ route('alumni.jobs.index', ['job_type' => 'internship']) }}" class="@if(request('job_type') === 'internship') bg-primary text-white @else bg-white border border-gray-300 text-gray-700 @endif px-4 py-2 rounded-full text-sm hover:bg-gray-50 transition-colors whitespace-nowrap">
                    Internships
                </a>
                <a href="{{ route('alumni.jobs.index', ['job_type' => 'other']) }}" class="@if(request('job_type') === 'other') bg-primary text-white @else bg-white border border-gray-300 text-gray-700 @endif px-4 py-2 rounded-full text-sm hover:bg-gray-50 transition-colors whitespace-nowrap">
                    Contract/Project
                </a>
            </div>
        </div>
        ...
        <div class="flex flex-col sm:flex-row sm:justify-between sm:items-center mb-6 gap-4">
            ...
            <div class="flex items-center space-x-2">
                ...
                <select id="sort-by" onchange="window.location.href=this.value" class="px-3 py-1 border border-gray-300 rounded text-sm focus:outline-none focus:ring-2 focus:ring-primary">
                    <option value="{{ route('alumni.jobs.index', array_merge(request()->except('sort'), ['sort' => 'newest'])) }}" @if(request('sort') === 'newest' || !request('sort')) selected @endif>Newest First</option>
                    <option value="{{ route('alumni.jobs.index', array_merge(request()->except('sort'), ['sort' => 'oldest'])) }}" @if(request('sort') === 'oldest') selected @endif>Oldest First</option>
                    <option value="{{ route('alumni.jobs.index', array_merge(request()->except('sort'), ['sort' => 'salary-high'])) }}" @if(request('sort') === 'salary-high') selected @endif>Salary Range</option>
                </select>
            </div>
        </div>
        ...
        <div class="space-y-6">
            @forelse($jobs as $job)
                <div class="bg-white rounded-lg shadow-sm p-4 sm:p-6 @if($job->is_featured) border-l-4 border-blue-500 @endif hover:shadow-md transition-shadow">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-2 sm:gap-3 mb-4 text-sm text-gray-600">
                        ...
                        <div class="flex items-start sm:items-center gap-1 flex-wrap">
                            <svg class="w-4 h-4 mr-1.5 flex-shrink-0 mt-0.5 sm:mt-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 13.255A23.931 23.931 0 0112 15c-3.183 0-6.22-.62-9-1.745M16 6V4a2 2 0 00-2-2h-4a2 2 0 00-2-2v2m8 0V6a2 2 0 012 2v6a2 2 0 01-2 2H8a2 2 0 01-2-2V8a2 2 0 012-2V6"></path>
                            </svg>
                            <span class="break-words">{{ $job->department?->title ?? 'N/A' }}</span>
                            @if($job->department)
                                <span class="text-xs bg-blue-50 text-blue-700 px-2 py-0.5 rounded">{{ $job->department->formatted_code }}</span>
                            @endif
                        </div>
                    ...
                    <div class="flex justify-start">
                        <a href="{{ route('alumni.jobs.show', $job->id) }}" class="w-full sm:w-auto px-6 py-2 bg-primary text-white text-sm rounded-lg hover:bg-blue-700 text-center font-medium transition-colors">
                            View Details
                        </a>
                    </div>
                </div>
            @empty
                <div class="bg-white rounded-lg shadow-sm p-8 sm:p-12 text-center">
                    ...
                    <a href="{{ route('alumni.jobs.index') }}" class="inline-block px-6 py-2 bg-primary text-white rounded-lg hover:bg-blue-700 transition-colors font-medium">
