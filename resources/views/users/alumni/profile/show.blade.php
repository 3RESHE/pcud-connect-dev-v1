...
<div class="w-full bg-gray-50 min-h-screen py-4 sm:py-6 lg:py-8">
    <div class="w-full max-w-5xl mx-auto px-3 sm:px-4 md:px-6 lg:px-8">
        ...
        @if ($profile && !$profile->profile_complete)
            <div class="mb-6 bg-yellow-50 border-l-4 border-yellow-400 p-3 sm:p-4 rounded-r">
                <div class="flex gap-3">
                    ...
                    <div class="flex-1 min-w-0">
                        <p class="text-xs sm:text-sm text-yellow-800 break-words"><strong>Incomplete Profile:</strong> Complete your profile to unlock more opportunities.</p>
                        <div class="w-full bg-gray-200 rounded-full h-2 mt-2">
                            <div class="bg-blue-600 h-2 rounded-full" style="width: {{ $profile->getProfileCompletionPercentage() }}%"></div>
                        </div>
                        <p class="text-xs text-yellow-700 mt-1">{{ $profile->getProfileCompletionPercentage() }}% Complete</p>
                    </div>
                </div>
            </div>
        @endif
        ...
        @if ($profile)
            ...
            <div class="bg-white shadow-sm rounded-lg overflow-hidden mb-6 sm:mb-8">
                <div class="px-3 sm:px-4 md:px-6 py-6 sm:py-8 bg-gradient-to-r from-blue-600 to-blue-700">
                    <div class="flex flex-col sm:flex-row gap-4 sm:gap-6">
                        <div class="flex-shrink-0">
                            @if ($profile->profile_photo)
                                <img src="{{ asset('storage/' . $profile->profile_photo) }}" alt="Profile Photo"
                                    class="w-24 h-24 sm:w-32 sm:h-32 rounded-full object-cover border-4 border-white">
                            @else
                                <div class="w-24 h-24 sm:w-32 sm:h-32 bg-gray-300 rounded-full flex items-center justify-center border-4 border-white">
                        ...
                        <div class="flex-1 min-w-0 text-white">
                            <h2 class="text-2xl sm:text-3xl font-bold break-words">{{ $profile->getFullName() }}</h2>
                            <p class="text-base sm:text-lg mt-2 break-words">{{ $profile->headline ?? 'No headline' }}</p>
                            <div class="flex flex-wrap gap-2 mt-3">
                                @if ($profile->graduation_year)
                                    <span class="inline-block px-3 py-1 bg-blue-500 rounded-full text-xs sm:text-sm">
                                        {{ $profile->getGraduationYearDisplay() }}
                                    </span>
                                @endif
                                @if ($profile->honors)
                                    <span class="inline-block px-3 py-1 bg-yellow-500 rounded-full text-xs sm:text-sm">
                                        {{ $profile->honors }}
                                    </span>
                                @endif
                                @if ($profile->current_position)
                                    <span class="inline-block px-3 py-1 bg-green-500 rounded-full text-xs sm:text-sm">
                ...
                <!-- Contact & Location -->
                <div class="px-3 sm:px-4 md:px-6 py-4 sm:py-6 border-t border-gray-200 grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div class="min-w-0">
                        <p class="text-xs sm:text-sm text-gray-600 break-words">Email</p>
                        <p class="text-sm sm:text-base text-gray-900 mt-1 break-words">
                            <a href="mailto:{{ $profile->getEmail() }}" class="text-blue-600 hover:underline">{{ $profile->getEmail() }}</a>
                        </p>
                    </div>
                    <div class="min-w-0">
                    ...
                    <div class="min-w-0">
                        <p class="text-xs sm:text-sm text-gray-600 break-words">Location</p>
                        <p class="text-sm sm:text-base text-gray-900 mt-1 break-words">{{ $profile->current_location ?? 'Not specified' }}</p>
                    </div>
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4 sm:gap-6 mb-6 sm:mb-8">
                ...
                <div class="bg-white shadow-sm rounded-lg p-4 sm:p-6">
                    <h3 class="text-xs sm:text-sm font-semibold text-gray-600 uppercase break-words">Current Organization</h3>
                    <p class="text-base sm:text-lg font-bold text-gray-900 mt-2 break-words">{{ $profile->current_organization ?? 'Not specified' }}</p>
                </div>

                <!-- Industry -->
                <div class="bg-white shadow-sm rounded-lg p-4 sm:p-6">
                    <h3 class="text-xs sm:text-sm font-semibold text-gray-600 uppercase break-words">Industry</h3>
                    <p class="text-base sm:text-lg font-bold text-gray-900 mt-2 break-words">{{ $profile->current_industry ?? 'Not specified' }}</p>
                </div>

                <!-- GWA -->
                <div class="bg-white shadow-sm rounded-lg p-4 sm:p-6">
                    <h3 class="text-xs sm:text-sm font-semibold text-gray-600 uppercase break-words">GWA</h3>
                    <p class="text-base sm:text-lg font-bold text-gray-900 mt-2 break-words">{{ $profile->getGWADisplay() }}</p>
                </div>
            </div>
            ...
            @if ($profile->professional_summary)
                <div class="bg-white shadow-sm rounded-lg overflow-hidden mb-6 sm:mb-8">
                    <div class="px-3 sm:px-4 md:px-6 py-3 sm:py-4 border-b border-gray-200 bg-gray-50">
                        <h3 class="text-base sm:text-lg font-semibold text-gray-900 break-words">Professional Summary</h3>
                    </div>
                    <div class="px-3 sm:px-4 md:px-6 py-4 sm:py-6">
                        <p class="text-sm sm:text-base text-gray-700 break-words whitespace-pre-wrap">{{ $profile->professional_summary }}</p>
                    </div>
                </div>
            @endif
                ...
                <div class="bg-white shadow-sm rounded-lg overflow-hidden mb-6 sm:mb-8">
                    ...
                    <div class="px-3 sm:px-4 md:px-6 py-4 sm:py-6 space-y-4 sm:space-y-6">
                        @if ($profile->technical_skills)
                            <div class="min-w-0">
                                <p class="text-xs sm:text-sm font-semibold text-gray-600 uppercase break-words">Technical Skills</p>
                                <div class="flex flex-wrap gap-2 mt-2">
                                    @foreach ($profile->getTechnicalSkillsArray() as $skill)
                                        <span class="inline-block px-3 py-1 bg-blue-100 text-blue-800 rounded-full text-xs sm:text-sm">
                                            {{ $skill }}
                                        </span>
                                    @endforeach
                                </div>
                        ...
                        @if ($profile->soft_skills)
                            <div class="min-w-0">
                                <p class="text-xs sm:text-sm font-semibold text-gray-600 uppercase break-words">Soft Skills</p>
                                <div class="flex flex-wrap gap-2 mt-2">
                        ...
                        @if ($profile->languages)
                            <div class="min-w-0">
                                <p class="text-xs sm:text-sm font-semibold text-gray-600 uppercase break-words">Languages</p>
                                <div class="flex flex-wrap gap-2 mt-2">
                                    @foreach ($profile->getLanguagesArray() as $lang)
                        ...
                        @foreach ($experiences as $exp)
                            <div class="border-b border-gray-200 pb-4 sm:pb-6 last:border-b-0 last:pb-0">
                                <div class="flex flex-col sm:flex-row sm:items-start gap-2 sm:gap-3 mb-2">
                                    <div class="flex-1 min-w-0">
                                        <h4 class="text-sm sm:text-base font-semibold text-gray-900 break-words">{{ $exp->role_position }}</h4>
                                        <p class="text-xs sm:text-sm text-gray-600 break-words">{{ $exp->organization }}</p>
                                        @if ($exp->location)
                                            <p class="text-xs sm:text-sm text-gray-500 break-words">📍 {{ $exp->location }}</p>
                                        @endif
                                    </div>
                                    @if ($exp->is_current)
                                        <span class="inline-block px-3 py-1 bg-green-100 text-green-800 text-xs font-medium rounded-full whitespace-nowrap">
                                            Current
                                        </span>
                                    @endif
                                </div>
                        ...
                        @foreach ($projects as $proj)
                            <div class="border-b border-gray-200 pb-4 sm:pb-6 last:border-b-0 last:pb-0">
                                <div class="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-2 mb-2">
                                    <div class="flex-1 min-w-0">
                                        <h4 class="text-sm sm:text-base font-semibold text-gray-900 break-words">{{ $proj->title }}</h4>
                                        @if ($proj->url)
                                ...
                                <p class="text-xs sm:text-sm text-gray-500 mb-2 break-words">
                                    ...
                                    —
                                    {{ $proj->end_date?->format('M Y') ?? 'Present' }}
                                </p>
            ...
            @if ($profile->linkedin_url || $profile->github_url || $profile->portfolio_url)
                <div class="bg-white shadow-sm rounded-lg overflow-hidden">
                    ...
                    <div class="px-3 sm:px-4 md:px-6 py-4 sm:py-6 space-y-3">
        ...
        @else
            ...
            <div class="bg-white shadow-sm rounded-lg p-6 sm:p-8 text-center">
                ...
                <p class="text-xs sm:text-sm text-gray-600 mb-4 break-words">Create your alumni profile to connect with other graduates and showcase your career journey.</p>
                <a href="{{ route('alumni.profile.edit') }}" class="inline-block px-4 sm:px-6 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition text-sm sm:text-base whitespace-nowrap">
                    Create Profile
                </a>
            </div>
        @endif
