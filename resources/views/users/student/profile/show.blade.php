...
<div class="w-full bg-gray-50 min-h-screen py-4 sm:py-6 lg:py-8">
    <div class="w-full max-w-5xl mx-auto px-3 sm:px-4 md:px-6 lg:px-8">
        ...
        @if ($profile)
                ...
                <div class="px-3 sm:px-4 md:px-6 py-4 sm:py-6 space-y-4 sm:space-y-6">
                    ...
                    <div class="flex flex-col sm:flex-row gap-4 sm:gap-6">
                        ...
                        <div class="flex-1 min-w-0">
                            <div class="mb-4">
                                <p class="text-xs sm:text-sm text-gray-600 break-words">Brief Summary</p>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6 pt-4 border-t border-gray-200">
                        ...
                        <div class="min-w-0">
                            ...
                            <p class="text-sm sm:text-base text-gray-900 mt-1 capitalize break-words">
                                {{ str_replace('_', ' ', $profile->gender ?? 'Not specified') }}
                            </p>
                        </div>
                        <div class="min-w-0">
                ...
                <div class="bg-white shadow-sm rounded-lg mb-6 sm:mb-8 overflow-hidden">
                    ...
                    <div class="px-3 sm:px-4 md:px-6 py-4 sm:py-6 space-y-4">
                        @if ($profile->linkedin_url)
                            <a href="{{ $profile->linkedin_url }}" target="_blank" rel="noopener noreferrer" class="flex items-center gap-3 text-blue-600 hover:underline break-words">
                        ...
                        @if ($profile->github_url)
                            <a href="{{ $profile->github_url }}" target="_blank" rel="noopener noreferrer" class="flex items-center gap-3 text-blue-600 hover:underline break-words">
                                <svg class="w-5 h-5 flex-shrink-0" fill="currentColor" viewBox="0 0 24 24">
                        ...
                        @if ($profile->portfolio_url)
                            <a href="{{ $profile->portfolio_url }}" target="_blank" rel="noopener noreferrer" class="flex items-center gap-3 text-blue-600 hover:underline break-words">
            ...
            <!-- Skills & Competencies -->
            @if ($profile->technical_skills || $profile->soft_skills || $profile->certifications || $profile->languages || $profile->hobbies)
                <div class="bg-white shadow-sm rounded-lg mb-6 sm:mb-8 overflow-hidden">
                    <div class="px-3 sm:px-4 md:px-6 py-3 sm:py-4 border-b border-gray-200 bg-gray-50">
                    ...
                    <div class="px-3 sm:px-4 md:px-6 py-4 sm:py-6 space-y-4 sm:space-y-6">
                        @if ($profile->technical_skills)
                            <div class="min-w-0">
                                ...
                                <div class="flex flex-wrap gap-2 mt-2">
                                    @foreach (array_filter(array_map('trim', explode(',', $profile->technical_skills))) as $skill)
                                        <span class="px-3 py-1 bg-blue-100 text-blue-800 text-xs font-medium rounded-full">{{ $skill }}</span>
                                    @endforeach
                                </div>
                            </div>
                        @endif
                        ...
                        @if ($profile->soft_skills)
                            <div class="min-w-0">
                                ...
                                <div class="flex flex-wrap gap-2 mt-2">
                                    @foreach (array_filter(array_map('trim', explode(',', $profile->soft_skills))) as $skill)
                                        <span class="px-3 py-1 bg-green-100 text-green-800 text-xs font-medium rounded-full">{{ $skill }}</span>
                                    @endforeach
                                </div>
                            </div>
                        @endif
                        ...
                        @if ($profile->languages)
                            <div class="min-w-0">
                                ...
                                <div class="flex flex-wrap gap-2 mt-2">
                                    @foreach (array_filter(array_map('trim', explode(',', $profile->languages))) as $lang)
                                        <span class="px-3 py-1 bg-purple-100 text-purple-800 text-xs font-medium rounded-full">{{ $lang }}</span>
                                    @endforeach
                                </div>
                            </div>
                        @endif
                        ...
                        @if ($profile->hobbies)
                            <div class="min-w-0">
                                <p class="text-xs sm:text-sm text-gray-600 break-words font-medium">Hobbies & Interests</p>
                                <p class="text-sm sm:text-base text-gray-900 mt-2 break-words">{{ $profile->hobbies }}</p>
                            </div>
                        @endif
                    </div>
                </div>
                    ...
                    <div class="px-3 sm:px-4 md:px-6 py-4 sm:py-6">
                        <div class="space-y-2">
                            @foreach ($profile->resumes as $resume)
                                <a href="{{ asset('storage/' . $resume) }}" target="_blank" class="flex items-center gap-3 p-3 bg-blue-50 rounded-lg hover:bg-blue-100 transition">
                                    <svg class="w-5 h-5 text-blue-600 flex-shrink-0" fill="currentColor" viewBox="0 0 20 20">
                                        <path d="M8 16.5a1 1 0 11-2 0 1 1 0 012 0zM15 7H4V5h11v2zM15 11H4V9h11v2z"></path>
                                    </svg>
                                    <span class="text-sm text-blue-600 font-medium break-all flex-1">
                                        {{ basename($resume) }}
                                    </span>
                    ...
                    <div class="px-3 sm:px-4 md:px-6 py-4 sm:py-6">
                        <div class="space-y-2">
                            @foreach ($profile->certifications as $cert)
                                <a href="{{ asset('storage/' . $cert) }}" target="_blank" class="flex items-center gap-3 p-3 bg-green-50 rounded-lg hover:bg-green-100 transition">
                                    ...
                                    <span class="text-sm text-green-600 font-medium break-all flex-1">
                                        {{ basename($cert) }}
                                    </span>
                            ...
                            <div class="border-b border-gray-200 pb-4 sm:pb-6 last:border-b-0 last:pb-0">
                                <div class="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-2 mb-2">
                                    <div class="flex-1 min-w-0">
                                        ...
                                        @if ($proj->url)
                                            <a href="{{ $proj->url }}" target="_blank" rel="noopener noreferrer" class="text-blue-600 hover:underline text-xs sm:text-sm break-words">
                                                View Project →
                                            </a>
                                        @endif
                                    </div>
